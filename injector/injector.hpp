...
namespace injector
{
    ...
    protected:
        ...
        template <class T>
        friend union basic_memory_pointer;
    ...
    public:
        ...
        template <class T>
        T *get() const { return (T *)this->p; }
        template <class T>
        T *get_raw() const { return (T *)this->p; }
    ...
    public:
        ...
        template <class T>
        T *get_raw() const { return auto_pointer(p); }
    ...
    struct scoped_unprotect
    {
        ...
        scoped_unprotect(memory_pointer_tr addr, size_t size) : addr(addr.get<void>()),
                                                                size(size)
        {
            if (size == 0)
            {
