...
static CSprite2d** CRadar__RadarSprites;
CSprite2d** CRadar::RadarSprites = (CSprite2d**)CRadar__RadarSprites; // [130]

static rage::Vector2* CRadar__vec2DRadarOrigin;
rage::Vector2& vec2DRadarOrigin = *(rage::Vector2*)CRadar__vec2DRadarOrigin;

static uint32_t CRadar__IsRenderPhaseTime;
bool CRadar::IsRenderPhaseTime() {
    return plugin::CallAndReturnDyn<bool>(CRadar__IsRenderPhaseTime);
}
...
void plugin::InitPatterns<CRadar>() {
    ...
    CRadar__RadarSprites = (CSprite2d**)plugin::patch::GetPointer(plugin::GetPattern("89 0C 85 ? ? ? ? A3 ? ? ? ? 48 C3 A1 ? ? ? ? 33 C9 89 0C 85 ? ? ? ? 40 A3 ? ? ? ? 48 C3 CC", 3));
    CRadar__vec2DRadarOrigin = (rage::Vector2*)plugin::patch::GetPointer(plugin::GetPattern("A3 ? ? ? ? A1 ? ? ? ? 6A 00 6A FF", 1));

    CRadar__IsRenderPhaseTime = plugin::GetPattern("80 3D ? ? ? ? ? 75 74 80 3D", 0);
}
